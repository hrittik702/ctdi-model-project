...
8. **[Dataset Provenance & Acquisition Audit](provenance.md)**: Comprehensive unbroken chain of custody ($\text{Source} \to \text{URL} \to \text{Raw File} \to \text{Audit Findings} \to \text{Decision} \to \text{Data Used}$) across Air Quality, Meteorology, Visibility substitution, Traffic, cryptographic checksums, and empirical missingness figures (Figures 6–9).
